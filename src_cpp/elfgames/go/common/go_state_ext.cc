...
void GoStateExt::showFinishInfo(FinishReason reason) const {
  ...
  std::cerr << _state.showBoard() << std::endl;
  std::string sgf_record = dumpSgf("");
  std::cerr << sgf_record << std::endl;

  std::cerr << "[" << _game_idx << ":" << _seq
  ...
  switch (reason) {
    case FR_RESIGN:
      std::cerr << "Player " << player2str(player) << " resigned at "
                << _state.getPly()
                << " Resign Thres: " << _resign_check.resign_thres;
      break;
    case FR_MAX_STEP:
      std::cerr << "Ply: " << _state.getPly()
                << " exceeds thread_state.Restarting the game";
      break;
    case FR_TWO_PASSES:
      std::cerr << "Both pass at " << _state.getPly();
      break;
    case FR_ILLEGAL:
      std::cerr << "Illegal move at " << _state.getPly();
      break;
    case FR_CLEAR:
      std::cerr << "Restarting at " << _state.getPly();
      break;
    case FR_CHEAT_NEWER_WINS_HALF:
      std::cerr << "Cheat mode: Version: " << curr_request_.vers.info()
                << ", swap: " << curr_request_.client_ctrl.player_swap;
      break;
    case FR_CHEAT_SELFPLAY_RANDOM_RESULT:
      std::cerr << "Cheat selfplay mode: Version: " << curr_request_.vers.info()
                << ", swap: " << curr_request_.client_ctrl.player_swap;
      break;
  }
  std::cerr << ", Value: " << _state.getFinalValue()
